...
Pace still controls the shared timeline. The amplitude slider is labelled Other faces: the four image-driven faces retain their authored small amplitudes. The other seventeen faces keep their previous geometric motion, with tooth/tongue shading removed to match the toothless artwork direction. No audio/model behavior changes. Static serving explicitly allows only the named PNG files in the four active libraries and recovered red-eyes-first11 collection. Restart Start.command once after adding the first11 routes, then refresh the browser.
